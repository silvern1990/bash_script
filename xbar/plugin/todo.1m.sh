...
list_items() {
    todo.sh ls  | awk 'NR>1 {print buf}{buf = $0}' | sed '/--/d' | awk ' { print $0,"| bash=/opt/homebrew/Cellar/todo-txt/2.13.0/bin/todo.sh param1=do param2="$1" terminal=false refresh=true" }'
}
